...
class TestContextAccessFunctions:
    ...
    def test_get_input_data_from_context(self):
        ...
        with patch('glovebox.cli.helpers.parameter_helpers.get_input_result_from_context', return_value=input_result):
    ...
    def test_get_input_data_from_context_none(self):
        ...
        with patch('glovebox.cli.helpers.parameter_helpers.get_input_result_from_context', return_value=None):
            data = get_input_data_from_context(ctx)
            assert data is None
    ...
    def test_get_input_path_from_context(self):
        ...
        ctx = Mock(spec=typer.Context)
        input_result = InputResult(raw_value="test.json", resolved_path=Path("test.json"))
    ...
    def test_get_output_path_from_context(self):
        ...
        ctx = Mock(spec=typer.Context)
        output_result = OutputResult(raw_value="output.json", resolved_path=Path("output.json"))
        
        with patch('glovebox.cli.helpers.parameter_helpers.get_output_result_from_context', return_value=output_result):
    ...
    def test_get_format_type_from_context(self):
        ...
        with patch('glovebox.cli.helpers.parameter_helpers.get_format_result_from_context', return_value=format_result):
    ...
    def test_get_format_type_from_context_default(self):
        ...
        with patch('glovebox.cli.helpers.parameter_helpers.get_format_result_from_context', return_value=None):
            format_type = get_format_type_from_context(ctx)
...
class TestInputParameterProcessing:
    ...
    def test_read_input_from_result_stdin(self):
        ...
        with patch('glovebox.cli.helpers.parameter_helpers.read_input_data', return_value="stdin data") as mock_read:
            data = read_input_from_result(result)
...
class TestOutputParameterProcessing:
    ...
    def test_process_output_parameter_existing_file_confirm(self, mock_confirm, tmp_path):
...
class TestFormatParameterProcessing:
    ...
    def test_format_and_output_data(self, mock_create_formatter, tmp_path):
        ...
        # Setup results
        output_file = tmp_path / "output.json"
        output_result = OutputResult(raw_value=str(output_file), resolved_path=output_file)
        format_result = FormatResult(format_type="json")
        
        test_data = {"test": "data"}
        
        with patch('glovebox.cli.helpers.parameter_helpers.write_output_from_result') as mock_write:
...
class TestValidationFunctions:
    ...
    def test_validate_output_path_create_dirs_permission_error(self, mock_mkdir, tmp_path):
    ...
    def test_validate_output_path_no_write_permission(self, mock_unlink, mock_touch, tmp_path):
