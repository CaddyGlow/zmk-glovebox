...
{%- endif %}
    ...
    def test_variable_editing_preserves_templates(self, sample_template_layout):
        ...
        updated_data = self.set_field_value(updated_data, "variables.build_info.version", "2.0.0")
        updated_data = self.set_field_value(updated_data, "variables.features.combos", False)
    ...
    def test_template_processing_after_edits(self, sample_template_layout):
        ...
        assert "PostEditUser <postedituser@test.com>" == layout.creator
    ...
    def test_adding_new_variables_and_templates(self, sample_template_layout):
        ...
        # Add new variables
        updated_data = self.set_field_value(data, "variables.device.name", "CustomDevice")
        updated_data = self.set_field_value(updated_data, "variables.device.version", "3.1.4")
...
Build: {{ variables.build_info.version }}
        ...
        for case in test_cases:
            test_data = self.set_field_value(updated_data, "variables.features.combos", case["combos"])
            test_data = self.set_field_value(test_data, "variables.features.macros", case["macros"])
            
            layout = LayoutData.load_with_templates(test_data)
            assert case["expected"] in layout.creator
    ...
    def test_multiple_edit_cycles_stability(self, sample_template_layout):
        ...
        for cycle in range(3):
            ...
            updated_data = self.set_field_value(data, "variables.user_name", f"CycleUser{cycle}")
            updated_data = self.set_field_value(updated_data, "variables.timing.fast", 180 + cycle * 20)
            updated_data = self.set_field_value(updated_data, "variables.build_info.version", f"1.{cycle}.0")
    ...
    def test_roundtrip_edit_template_edit(self, sample_template_layout):
        ...
        step2_data = self.set_field_value(step2_data, "variables.user_name", "Step2User")
    ...
    def test_compilation_workflow_integration(self, sample_template_layout):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Edit variables
            updated_data = self.set_field_value(data, "variables.user_name", "CompileUser")
            updated_data = self.set_field_value(updated_data, "variables.timing.fast", 190)
