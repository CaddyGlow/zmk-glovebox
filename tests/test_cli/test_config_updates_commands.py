...
class TestDisableUpdatesCommand:
    """Test the disable-updates command."""

    @patch("glovebox.core.version_check.create_zmk_version_checker")
    @patch("glovebox.cli.commands.config.updates.print_success_message")
    def test_disable_updates_success(
        self,
        mock_print_success,
        mock_create_checker,
        cli_runner,
        ...
        """Test successful disable-updates command."""
        mock_create_checker.return_value = mock_version_checker

        result = cli_runner.invoke(app, ["config", "disable-updates"])

        assert result.exit_code == 0
        mock_version_checker.disable_version_checks.assert_called_once()
        mock_print_success.assert_called_once_with("ZMK version checks disabled")

    @patch("glovebox.core.version_check.create_zmk_version_checker")
    def test_disable_updates_exception(
        self,
        mock_create_checker,
        cli_runner,
        mock_version_checker,
        ...
        """Test disable-updates when an exception occurs."""
        mock_create_checker.return_value = mock_version_checker
        mock_version_checker.disable_version_checks.side_effect = Exception(
            "Config error"
        )

        # The @handle_errors decorator should catch and handle the exception
        result = cli_runner.invoke(app, ["config", "disable-updates"])
...
class TestEnableUpdatesCommand:
    """Test the enable-updates command."""

    @patch("glovebox.core.version_check.create_zmk_version_checker")
    @patch("glovebox.cli.commands.config.updates.print_success_message")
    def test_enable_updates_success(
        self,
        mock_print_success,
        mock_create_checker,
        cli_runner,
        ...
        """Test successful enable-updates command."""
        mock_create_checker.return_value = mock_version_checker

        result = cli_runner.invoke(app, ["config", "enable-updates"])

        assert result.exit_code == 0
        mock_version_checker.enable_version_checks.assert_called_once()
        mock_print_success.assert_called_once_with("ZMK version checks enabled")

    @patch("glovebox.core.version_check.create_zmk_version_checker")
    def test_enable_updates_exception(
        self,
        mock_create_checker,
        cli_runner,
        mock_version_checker,
        ...
        """Test enable-updates when an exception occurs."""
        mock_create_checker.return_value = mock_version_checker
        mock_version_checker.enable_version_checks.side_effect = Exception(
            "Config error"
        )
...
class TestConfigUpdatesIntegration:
    """Integration tests for config updates commands."""

    @patch("glovebox.core.version_check.create_zmk_version_checker")
    @patch("glovebox.cli.commands.config.updates.print_success_message")
    def test_disable_then_check_then_enable_workflow(
        self,
        mock_print_success,
        mock_create_checker,
        cli_runner,
        ...
        """Test complete workflow: disable → check → enable."""
        mock_create_checker.return_value = mock_version_checker

        # First disable
        mock_version_checker.check_for_updates.return_value = VersionCheckResult(
            has_update=False, check_disabled=True
        )

        disable_result = cli_runner.invoke(app, ["config", "disable-updates"])
        assert disable_result.exit_code == 0

        # Then check (should show disabled message)
        check_result = cli_runner.invoke(app, ["config", "check-updates"])
        assert check_result.exit_code == 0
        assert "Version checks are disabled" in check_result.output

        # Then enable
        enable_result = cli_runner.invoke(app, ["config", "enable-updates"])
