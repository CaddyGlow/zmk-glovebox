"""Tests for KeymapComponentService."""
...
class TestKeymapComponentService:
    """Test KeymapComponentService functionality."""
    ...
    def service(self, mock_file_adapter):
        """Create a KeymapComponentService for testing."""
        return KeymapComponentService(mock_file_adapter)

    @pytest.fixture
    def sample_keymap_data(self, sample_keymap_json):
        """Create a sample KeymapData model for testing."""
        return KeymapData.model_validate(sample_keymap_json)
    ...
    def test_initialization(self, mock_file_adapter):
        """Test KeymapComponentService initialization."""
        service = KeymapComponentService(mock_file_adapter)
        assert service._file_adapter == mock_file_adapter
        assert service._service_name == "KeymapComponentService"
    ...
    def test_extract_components_success(self, service, sample_keymap_data, output_dir):
        ...
        service._file_adapter.write_json.assert_any_call(
            ...
            sample_keymap_data.model_dump(
                mode="json",
                by_alias=True,
                include=set(KeymapMetadata.model_fields.keys()),
            ),
        )
    ...
    def test_extract_components_error(self, service, sample_keymap_data, output_dir):
        ...
        with pytest.raises(KeymapError, match="Layer extraction failed"):
    ...
    def test_combine_components_success(self, service, sample_keymap_data, output_dir):
        ...
        def mock_process_layers(keymap, layers_dir):
            ...
            keymap.layers = [
                ...
                [
                    KeymapBinding(value="&kp", params=[KeymapParam(value="1")])
                    for _ in range(80)
                ],
                [
                    KeymapBinding(value="&kp", params=[KeymapParam(value="F1")])
                    for _ in range(80)
                ],
            ]

        with patch.object(
            service, "_process_layers_for_combination", side_effect=mock_process_layers
        ):
            layers_dir = output_dir / "layers"
            ...
            assert isinstance(combined_keymap, KeymapData)
    ...
    def test_combine_components_directory_not_found(self, service, sample_keymap_data):
        ...
        with (
            ...
            pytest.raises(KeymapError, match="Layers directory not found"),
        ):
    ...
    def test_process_layers_for_combination(
        ...
        with (
            ...
            service._process_layers_for_combination(combined_keymap, layers_dir)
    ...
    def test_create_keymap_component_service(self, mock_file_adapter):
        """Test factory function for creating KeymapComponentService."""
        # Test with provided file adapter
        service = create_keymap_component_service(mock_file_adapter)
        assert isinstance(service, KeymapComponentService)
        assert service._file_adapter == mock_file_adapter
        ...
        with patch("glovebox.adapters.file_adapter.create_file_adapter") as mock_create:
            ...
            service = create_keymap_component_service()
            assert isinstance(service, KeymapComponentService)
