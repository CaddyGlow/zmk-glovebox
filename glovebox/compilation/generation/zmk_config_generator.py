...
class ZmkConfigContentGenerator:
    ...
    def generate_config_workspace(
        ...
        try:
            ...
            if not self._generate_west_yml(workspace_path):
                return False
    ...
    def _create_build_yaml_content(self, shield_name: str, board_name: str) -> str:
        ...
        is_split = any(
            ...
            for indicator in ["corne", "crkbd", "lily58", "sofle", "kyria"]
        )

        if is_split:
            # Generate split keyboard build targets
...
"""
    ...
    def _generate_west_yml(self, workspace_path: Path) -> bool:
...
# Build firmware
west build -p always -b nice_nano_v2 -d build/left -- -DSHIELD={keyboard_profile.keyboard_name}_left
west build -p always -b nice_nano_v2 -d build/right -- -DSHIELD={keyboard_profile.keyboard_name}_right
```
