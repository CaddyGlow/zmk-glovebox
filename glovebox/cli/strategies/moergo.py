...
class MoergoStrategy(BaseCompilationStrategy):
    ...
    def _get_repository_branch(
        ...
        if hasattr(profile, 'firmware_version') and profile.firmware_version:
            ...
            if hasattr(firmware_config, 'version'):
                return firmware_config.version
    ...
    def _build_moergo_commands(self, params: CompilationParams) -> list[str]:
        ...
        # Run the Nix build
        commands.append("nix-build ./config --arg firmware 'import /src/default.nix {}' -j2 -o /tmp/combined --show-trace")

        # Install the firmware file with proper ownership
        commands.append('install -o "$UID" -g "$GID" /tmp/combined/glove80.uf2 ./glove80.uf2')
