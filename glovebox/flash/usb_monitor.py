...
class USBDeviceMonitorBase(abc.ABC):
    ...
    def wait_for_device(self, timeout: int = 60, poll_interval: float = 0.5) -> Optional[BlockDevice]:
...
class LinuxUSBDeviceMonitor(USBDeviceMonitorBase):
    ...
    def _update_mountpoints(self, device: BlockDevice, mount_points: dict[str, str]) -> None:
        ...
        def device_event(action, device):
            if action in ("add", "remove") and self.is_usb_device(device):
                if action == "add":
                    ...
                    with self._lock:
                        self.devices = [d for d in self.devices if d.path != device.device_node]
                ...
                for disk_name, disk_data in disk_info.items():
                    ...
                    for usb_device in usb_info:
                        ...
                        if (
                            ...
                            (vol_name_lower and (usb_name in vol_name_lower or vol_name_lower in usb_name)) or
                            # Match by media name
                            (media_name_lower and (usb_name in media_name_lower or media_name_lower in usb_name)) or
                            # Match by vendor in volume/media name
                            (vol_name_lower and usb_vendor and usb_vendor in vol_name_lower) or
                            (media_name_lower and usb_vendor and usb_vendor in media_name_lower)
                        ):
                            usb_data = usb_device
                            logger.debug(f"Matched USB device: {usb_device} to disk {disk_name}")
                            break
                    ...
                    device = BlockDevice(
                        ...
                        model=usb_data.get("name", disk_data.get("MediaName", "Unknown")) if usb_data else disk_data.get("MediaName", "Unknown"),
                        vendor=usb_data.get("vendor", "Unknown") if usb_data else "Unknown",
                        ...
                        mountpoints={volume_name: f"/Volumes/{volume_name}"} if volume_name in mounted_volumes else {}
                    )
                    
                    # Only add if it's a removable device or has USB info
                    if device.removable or usb_data:
                        self.devices.append(device)
                        logger.debug(f"Found device: {disk_name} - {device.model} (Vendor: {device.vendor}, Serial: {device.serial})")
    ...
    def _monitor_loop(self) -> None:
        ...
        while self._monitoring:
            ...
            for path in old_devices - new_devices:
                self._notify_callbacks("remove", BlockDevice(
                    name=Path(path).name,
                    device_node=path,
                    model="",
                    vendor="",
